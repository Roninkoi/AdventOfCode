...
pub fn day5_1(file_path: &str) -> io::Result<()> {
    ...
    for line in reader.by_ref().lines() {
        ...
        if l.is_empty() { // stack description ends
    ...
    for col in 0..stack_num { // transpose stack grid so that we can push and pop stacks
        let mut stack_col = Vec::<String>::new();
        for row in (0..stack_height).rev() {
            let v = &stack_grid[row][col];
            if !v.is_empty() && v != " " { // remove empty crates
                stack_col.push(v.clone());
            }
        }
        stacks.push(stack_col.clone());
    }

    for line in reader.by_ref().lines() { // parse and execute commands
        let l = line?;
        ...
        let mut took = Vec::<String>::new(); // take a number of crates from stack
        {
        ...
        {
            let stack_to = stacks.get_mut(to-1); // place a number of crates on a stack
            if stack_to.is_some() {
        ...
        for (i, s) in stacks.iter().enumerate() { // print state of stacks
            println!("{} {:?}", i + 1, s);
        }
    }

    for (i, s) in stacks.iter().enumerate() { // print top crates
